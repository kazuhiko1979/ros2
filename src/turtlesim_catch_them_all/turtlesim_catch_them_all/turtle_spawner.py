...
class TurtleSpawnerNode(Node):
    ...
    def call_spawn_service(self, x: float, y: float, theta: float, turtle_name: str):
        while not self.spawn_client_.wait_for_service(timeout_sec=1.0):
            self.get_logger().warn('Waitting for spawn service...')
    ...
    def call_kill_service(self, turtle_name: str):
        while not self.kill_client_.wait_for_service(timeout_sec=1.0):
            self.get_logger().warn('Waitting for kill service...')
    ...
    def callback_call_kill_service(self, future, turtle_name: str):
